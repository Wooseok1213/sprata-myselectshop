...
class UserProductMvcTest {
    ...
    private void mockUserSetup() {
        ...
        User testUser = new User(username, password, email, role); //테스트할 유저객체를 위의 값들로 생성시켜준다.
    ...
    void test2() throws Exception {
        ...
        signupRequestForm.add("username", "sollertia4351"); //add (Key, Value)
        signupRequestForm.add("password", "robbie1234");
        signupRequestForm.add("email", "sollertia@sparta.com");
        signupRequestForm.add("admin", "false");

        // when - then
        mvc.perform(post("/api/user/signup") //perform( 방식(get,post,put)
